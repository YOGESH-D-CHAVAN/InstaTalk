...
export default function Home() {
  ...
  const openChat = async (chat) => {
    ...
    try {
      ...
      // Update local state to clear unread badge immediately
      setChats(prevChats => 
        prevChats.map(c => {
          if (c._id === chat._id && c.unreadCounts) {
            const currentUserId = user?._id || user?.id;
            return {
              ...c,
              unreadCounts: { ...c.unreadCounts, [currentUserId]: 0 }
            };
          }
          return c;
        })
  ...
  const handleChatUpdate = (message) => {
     setChats(prevChats => {
        ...
        if (chatIndex > -1) {
           const updatedChat = { ...prevChats[chatIndex], updatedAt: message.createdAt || new Date() };
           
           // If we are NOT currently looking at this chat, increment unread locally
           if (selectedChat?._id !== message.chat) {
              const currentUserId = user?._id || user?.id;
              const counts = updatedChat.unreadCounts || {};
              updatedChat.unreadCounts = {
                 ...counts,
                 [currentUserId]: (counts[currentUserId] || 0) + 1
              };
           }
           ...
           // If chat isn't in list (new chat?), fetch all
           fetchChats();
           return prevChats;
        }
     });
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex justify-between items-center">
                    ...
                    {chat.unreadCounts && chat.unreadCounts[currentUserId] > 0 && !isSelected && (
                       <div className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm ml-2 uppercase tracking-wide">
                          New
                       </div>
                    )}
                  </div>
            ...
            return (
              <>
                ...
                <div className="flex-1 overflow-hidden relative">
                  <ChatBox chatId={selectedChat._id} user={user} key={selectedChat._id} onMessageReceived={handleChatUpdate} />
                </div>
              </>
            );
          })()
